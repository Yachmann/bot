...
URL = (
    ...
    "?data=1793415599000&evento_local=1775752182066x607042691407020000"
...
# Tipos de ingresso aceitos, em QUALQUER setor/categoria (Pista, Arquibancada,
# Cadeira Superior, Cadeira Inferior, Camarotes, etc.)
...
class _HealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot de ingressos (BTS) rodando.")
...
def is_target_key(key: str) -> bool:
    """
    Retorna True se a 'key' (formato 'Categoria||Tipo') tiver o tipo do
    ingresso igual a 'Inteira' ou 'Meia Estudante' (comparação
    case-insensitive), não importando o setor/categoria (Pista,
    Arquibancada, Cadeira Superior, Cadeira Inferior, Camarotes, etc.).
    """
    if "||" not in key:
        return False

    _categoria, tipo = key.split("||", 1)
    return tipo.strip().lower() in ALLOWED_TIPOS
...
def send_hourly_status():
    ...
    if cheapest_key:
        ...
        msg = (
            "✅ <b>Bot ativo (BTS).</b>\n\n"
            f"🎫 Menor preço encontrado (qualquer setor - Inteira/Meia Estudante):\n"
            f"{cheapest_key.replace('||', ' - ')}\n"
            f"💰 R${preco_reais:.2f}\n"
            f"📦 Disponíveis: {cheapest_available}"
        )
    else:
        msg = "✅ Bot ativo (BTS).\nNenhum ingresso Inteira/Meia Estudante encontrado."
    ...
    for key, info in matriz.items():
        ...
        if preco_reais < PRICE_THRESHOLD_REAIS and disponivel > 0:
            ...
            if alert_key not in already_alerted:
                msg = (
                    f"🎫 <b>Ingresso BTS abaixo de R${PRICE_THRESHOLD_REAIS:.0f}!</b>\n"
                    f"Tipo: {key.replace('||', ' - ')}\n"
